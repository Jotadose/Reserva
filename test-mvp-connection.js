...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF2eHdma2JjcnVuYWViYWhwbWZ0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTUzNTA4MzIsImV4cCI6MjA3MDkyNjgzMn0.MG6D8jFV0q-l0uxFP_3tQ-1ScXvmdckFe-T-_R9pJBA';
...
async function testMVPConnection() {
  ...
  try {
    ...
    if (usuariosError) {
      ...
      console.log('📋 Primeros usuarios:', usuarios?.map(u => `${u.nombre} (${u.email})`).join(', '));
    }
    ...
    if (barberosError) {
      ...
      console.log('📋 Barberos activos:', barberos?.map(b => `${b.nombre} - ${b.barberos[0]?.especialidades?.join(', ')}`).join('; '));
    }
    ...
    if (serviciosError) {
      ...
      console.log('📋 Servicios disponibles:', servicios?.map(s => `${s.nombre} - $${s.precio/100}`).join(', '));
    }
    ...
    if (reservasError) {
      ...
      if (reservas?.length > 0) {
        ...
        reservas.forEach(r => {
          console.log(`   • ${r.cliente?.nombre} → ${r.barbero?.nombre} → ${r.servicio?.nombre} (${r.fecha_hora})`);
        });
      }
    }
