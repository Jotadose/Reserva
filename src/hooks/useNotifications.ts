...
import { Notification, NotificationSettings, PushSubscription } from '../types/notifications';
...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Enviar notificación
  const sendNotification = useCallback((notificationData: Omit<Notification, 'id' | 'createdAt' | 'isRead'>) => {
  ...
  const markAsRead = useCallback((notificationId: string) => {
    ...
    addAuditLog('mark_notification_read', 'notifications', notificationId, []);
  }, [addAuditLog]);
  ...
  const updateSettings = useCallback((newSettings: Partial<NotificationSettings>) => {
    ...
    addAuditLog('update_notification_settings', 'settings', 'notifications', [
      { field: 'settings', oldValue: settings, newValue: { ...settings, ...newSettings } }
    ]);
  ...
  const subscribeToPush = useCallback(async (): Promise<PushSubscription | null> => {
    ...
    try {
      ...
      addToast({
        ...
        message: 'Recibirás notificaciones incluso cuando el sitio esté cerrado',
      });
        ...
        const randomNotifications = [
          {
            ...
            message: `${['María González', 'Pedro Ruiz', 'Sofia Martín'][Math.floor(Math.random() * 3)]} ha agendado una cita`,
          ...
          },
          {
            ...
            message: '¡Felicitaciones! Has alcanzado tu meta de ingresos del día',
