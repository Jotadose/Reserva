...
export type BookingStatus = 'confirmed' | 'pending' | 'in-progress' | 'completed' | 'cancelled' | 'no-show' | 'rescheduled';
    ...
    try {
      ...
      if (succeeded > 0) {
        addToast({
          ...
          message: `${succeeded} reserva(s) cancelada(s)${failed > 0 ? `, ${failed} falló(s)` : ''}`,
        });
      }
