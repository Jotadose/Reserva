...
export const AdvancedAnalytics: React.FC<AdvancedAnalyticsProps> = ({
  ...
  const { currentBookings, previousBookings } = useMemo(() => {
    ...
    const periodStart = new Date(
      now.getTime() - currentPeriod.days * 24 * 60 * 60 * 1000
    );
  ...
  const metrics: MetricData[] = useMemo(() => {
    ...
    const currentCompleted = currentBookings.filter(
      (b) => b.status === "completed"
    ).length;
    const previousCompleted = previousBookings.filter(
      (b) => b.status === "completed"
    ).length;

    const currentCancelled = currentBookings.filter(
      (b) => b.status === "cancelled"
    ).length;
    const previousCancelled = previousBookings.filter(
      (b) => b.status === "cancelled"
    ).length;

    const currentCancellationRate =
      currentBookings.length > 0
        ? (currentCancelled / currentBookings.length) * 100
        : 0;
    const previousCancellationRate =
      previousBookings.length > 0
        ? (previousCancelled / previousBookings.length) * 100
        : 0;
    ...
    const previousAvgBookingValue =
      previousBookings.length > 0
        ? previousRevenue / previousBookings.length
        : 0;

    // Clientes únicos
    const currentUniqueClients = new Set(
      currentBookings.map((b) => b.client.email)
    ).size;
    const previousUniqueClients = new Set(
      previousBookings.map((b) => b.client.email)
    ).size;
    ...
    const currentAvgDaysBetween =
      calculateAvgTimeBetweenBookings(currentBookings);
    const previousAvgDaysBetween =
      calculateAvgTimeBetweenBookings(previousBookings);
    ...
    const currentReturnClients = currentBookings.filter((booking) => {
      const clientPreviousBookings = bookings.filter(
        (b) =>
          b.client.email === booking.client.email &&
    ...
    const currentRetentionRate =
      currentBookings.length > 0
        ? (currentReturnClients / currentBookings.length) * 100
        : 0;
    ...
    const previousRetentionRate =
      previousBookings.length > 0
        ? (previousReturnClients / previousBookings.length) * 100
        : 0;
  ...
  const serviceAnalytics = useMemo(() => {
    const serviceStats = new Map<
      string,
      { count: number; revenue: number; avgPrice: number }
    >();

    currentBookings.forEach((booking) => {
      booking.services.forEach((service) => {
        const existing = serviceStats.get(service.name) || {
          count: 0,
          revenue: 0,
          avgPrice: 0,
        };
  ...
  const temporalAnalytics = useMemo(() => {
    ...
    const bestDay = Array.from(dayStats.entries()).sort(
      (a, b) => b[1] - a[1]
    )[0];
    const bestHour = Array.from(hourStats.entries()).sort(
      (a, b) => b[1] - a[1]
    )[0];
    ...
    return {
      ...
      hourDistribution: Array.from(hourStats.entries()).sort(
        (a, b) => a[0] - b[0]
      ),
    };
  }, [currentBookings]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="rounded-xl border border-gray-600 bg-gray-800/50 px-4 py-2 text-white backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
              {periods.map((period) => (
                <option
                  key={period.value}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        {metrics.slice(0, 4).map((metric) => {
          ...
          return (
            <div
              key={metric.label}
              className="group relative overflow-hidden rounded-2xl border border-gray-700 bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-6 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-2xl"
            >
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {metrics.slice(4).map((metric) => {
          ...
          return (
            <div
              key={metric.label}
              className="rounded-2xl border border-gray-700 bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-6 backdrop-blur-sm transition-all duration-300 hover:scale-105"
            >
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-400">
                  ...
                  <div className="mt-2 flex items-center">
                    {trendIcon}
                    <span className={`ml-1 text-xs ${trendColor}`}>
                      {Math.abs(
                        calculatePercentageChange(
                          metric.value,
                          metric.previousValue
      ...
      <div className="rounded-2xl border border-gray-700 bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-6 backdrop-blur-sm">
        <div className="mb-6 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-white flex items-center space-x-3">
            ...
            <span>Rendimiento por Servicio</span>
          </h3>
        ...
        {serviceAnalytics.length === 0 ? (
          <div className="py-8 text-center text-gray-400">
            <PieChart className="mx-auto mb-3 h-12 w-12 text-gray-600" />
            ...
            <table className="w-full">
              ...
              <tbody>
                {serviceAnalytics.map((service, index) => {
                  const totalRevenue = serviceAnalytics.reduce(
                    (sum, s) => sum + s.revenue,
                    0
                  );
                  const percentage =
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center">
                          <div
                            className={`mr-3 h-3 w-3 rounded-full ${getServiceColor(
                              index
                            )}`}
                          />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-gray-700 bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-6 backdrop-blur-sm">
          <h3 className="mb-4 text-lg font-semibold text-white flex items-center space-x-3">
              ...
              <div className="rounded-xl bg-blue-500/10 border border-blue-500/20 p-4">
                ...
                <p className="text-blue-400">
                  <span className="font-semibold capitalize">
                    {temporalAnalytics.bestDay.day}
                  </span>
                  <span className="ml-2 text-sm">
                    ({temporalAnalytics.bestDay.count} reservas)
                  </span>
                </p>
              </div>
            )}
              ...
              <div className="rounded-xl bg-green-500/10 border border-green-500/20 p-4">
                ...
                <p className="text-green-400">
                  <span className="font-semibold">
                    {temporalAnalytics.bestHour.hour}:00
                  </span>
                  <span className="ml-2 text-sm">
              ...
              <div className="space-y-2">
                {temporalAnalytics.dayDistribution.map(([day, count]) => {
                  const maxCount = Math.max(
                    ...temporalAnalytics.dayDistribution.map((d) => d[1])
                  );
                  const percentage =
                  ...
                  return (
                    <div key={day} className="flex items-center">
                      <div className="w-16 text-sm capitalize text-gray-400">
                        {day.slice(0, 3)}
                      </div>
          ...
          {temporalAnalytics.hourDistribution.length > 0 ? (
            <div className="space-y-2">
              {temporalAnalytics.hourDistribution.map(([hour, count]) => {
                const maxCount = Math.max(
                  ...temporalAnalytics.hourDistribution.map((h) => h[1])
                );
            ...
            <div className="py-8 text-center text-gray-400">
              ...
              <p>
                No hay datos suficientes para mostrar distribución por horas
              </p>
            </div>
          )}
        </div>
      ...
      {/* Resumen de Insights */}
      <div className="rounded-2xl border border-purple-700/30 bg-gradient-to-br from-purple-900/20 via-purple-800/20 to-indigo-900/20 p-6 backdrop-blur-sm">
        <h3 className="mb-4 text-lg font-semibold text-white flex items-center space-x-3">
          <div className="rounded-xl bg-purple-500/20 p-3">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-xl bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/20 p-4">
            <h4 className="mb-2 font-medium text-green-300 flex items-center space-x-2">
              <TrendingUp className="h-4 w-4" />
              <span>Crecimiento</span>
            </h4>
            <p className="text-sm text-gray-300">
              Los ingresos han{" "}
              {metrics[0].trend === "up" ? "aumentado" : "disminuido"} un{" "}
              {Math.abs(
                calculatePercentageChange(
                  metrics[0].value,
          ...
          <div className="rounded-xl bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 p-4">
            <h4 className="mb-2 font-medium text-blue-300 flex items-center space-x-2">
              <Users className="h-4 w-4" />
              <span>Retención</span>
            </h4>
            <p className="text-sm text-gray-300">
              El {metrics[5].value.toFixed(1)}% de tus clientes son recurrentes,
              lo que indica una buena fidelización.
            </p>
          </div>

          {serviceAnalytics.length > 0 && (
            <div className="rounded-xl bg-gradient-to-br from-yellow-500/10 to-yellow-600/10 border border-yellow-500/20 p-4">
              <h4 className="mb-2 font-medium text-yellow-300 flex items-center space-x-2">
          ...
          {temporalAnalytics.bestDay && (
            <div className="rounded-xl bg-gradient-to-br from-indigo-500/10 to-indigo-600/10 border border-indigo-500/20 p-4">
              <h4 className="mb-2 font-medium text-indigo-300 flex items-center space-x-2">
                <Calendar className="h-4 w-4" />
                <span>Mejor Día</span>
              </h4>
              <p className="text-sm capitalize text-gray-300">
                Los {temporalAnalytics.bestDay.day}s son tus días más ocupados
                con {temporalAnalytics.bestDay.count} reservas promedio.
              </p>
            </div>
          )}
        </div>
