...
export function BookingSystemMVP() {
  ...
  const { servicios, loading: serviciosLoading, formatearPrecio } = useServicios();
  ...
  const handleCreateReservation = async () => {
    try {
      ...
      const serviciosSeleccionados = servicios.filter(s => selectedServicios.includes(s.id_servicio));
      ...
      const reservaData = {
        ...
        notas: `Servicios: ${serviciosSeleccionados.map(s => s.nombre).join(', ')}`
      };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <p className="text-2xl font-bold text-blue-600">{barberos?.length || 0}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-semibold text-gray-600">Servicios</h3>
            <p className="text-2xl font-bold text-green-600">{servicios?.length || 0}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-semibold text-gray-600">Reservas</h3>
            <p className="text-2xl font-bold text-purple-600">{reservas?.length || 0}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-semibold text-gray-600">Usuarios</h3>
            <p className="text-2xl font-bold text-orange-600">{usuarios?.length || 0}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2">
                {servicios?.map((servicio) => (
                  <label key={servicio.id_servicio} className="flex items-center">
                    <input
                      type="checkbox"
                      checked={selectedServicios.includes(servicio.id_servicio)}
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedServicios([...selectedServicios, servicio.id_servicio]);
                        } else {
                          setSelectedServicios(selectedServicios.filter(id => id !== servicio.id_servicio));
                        }
                      }}
                      className="mr-2"
                    />
                    <span>{servicio.nombre} - {formatearPrecio ? formatearPrecio(servicio.precio) : `$${servicio.precio}`} ({servicio.duracion} min)</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <button
              ...
              disabled={!selectedBarbero || selectedServicios.length === 0 || !selectedDate || !clientData.nombre || reservasLoading}
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="space-y-3">
              {reservas && reservas.length > 0 ? reservas.slice(0, 5).map((reserva) => (
                <div key={reserva.id_reserva} className="p-3 bg-gray-50 rounded border">
                  <p className="font-medium">{reserva.cliente_nombre || 'Cliente'}</p>
                  <p className="text-sm text-gray-600">
                    {reserva.barbero_nombre || 'Barbero'} - {reserva.servicio_nombre || 'Servicio'}
                  </p>
                  <p className="text-sm text-gray-600">
                    {new Date(reserva.fecha_hora).toLocaleDateString()} a las {new Date(reserva.fecha_hora).toLocaleTimeString()}
                  </p>
