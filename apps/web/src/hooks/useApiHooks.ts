...
import { reservasApi, serviciosApi, usuariosApi, ApiResponse } from '../services/api';
...
export function useReservas() {
  ...
  const fetchReservas = useCallback(async (options?: {
    filters?: {
      id_barbero?: string;
      fecha_reserva?: string;
      estado?: string;
  ...
  const crearReserva = useCallback(async (data: Partial<Reserva>) => {
    setLoading(true);
    setError(null);
  ...
  const actualizarReserva = useCallback(async (id: string, updates: Partial<Reserva>) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (response.success) {
        ...
        const errorMsg = response.error?.message || 'Error al actualizar reserva';
        setError(errorMsg);
        throw new Error(errorMsg);
      }
    } catch (err: any) {
  ...
  const confirmarReserva = useCallback((id: string) => 
    actualizarReserva(id, { estado: 'confirmada' }), [actualizarReserva]);

  const completarReserva = useCallback((id: string, notas?: string) => 
    actualizarReserva(id, { estado: 'completada', notas_internas: notas }), [actualizarReserva]);

  const cancelarReserva = useCallback((id: string, motivo?: string) => 
    actualizarReserva(id, { estado: 'cancelada', motivo_cancelacion: motivo }), [actualizarReserva]);

  const marcarNoShow = useCallback((id: string) => 
    actualizarReserva(id, { estado: 'no_show' }), [actualizarReserva]);
...
export function useServicios() {
  ...
  const crearServicio = useCallback(async (data: Partial<Servicio>) => {
    setLoading(true);
    setError(null);
  ...
  const actualizarServicio = useCallback(async (id: string, updates: Partial<Servicio>) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (response.success) {
        ...
        const errorMsg = response.error?.message || 'Error al actualizar servicio';
        setError(errorMsg);
        throw new Error(errorMsg);
      }
    } catch (err: any) {
  ...
  const eliminarServicio = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setServicios(prev => prev.map(s => 
          s.id_servicio === id ? { ...s, activo: false } : s
        ));
        return true;
      } else {
        const errorMsg = response.error?.message || 'Error al eliminar servicio';
        setError(errorMsg);
        throw new Error(errorMsg);
      }
    } catch (err: any) {
  ...
  const getServiciosPorCategoria = useCallback((categoria: string) => {
...
export function useUsuarios() {
  ...
  const fetchUsuarios = useCallback(async (options?: {
  ...
  const crearUsuario = useCallback(async (data: Partial<Usuario>) => {
    setLoading(true);
    setError(null);
  ...
  const actualizarUsuario = useCallback(async (id: string, updates: Partial<Usuario>) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (response.success) {
        ...
        const errorMsg = response.error?.message || 'Error al actualizar usuario';
        setError(errorMsg);
        throw new Error(errorMsg);
      }
    } catch (err: any) {
  ...
  const eliminarUsuario = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (response.success) {
        // Marcar como inactivo en la lista local
        setUsuarios(prev => prev.map(u => 
          u.id_usuario === id ? { ...u, activo: false } : u
        ));
        return true;
      } else {
