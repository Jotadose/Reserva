...
async function runMigrationWithSupabase() {
  try {
    console.log(
      "🚀 Iniciando migración completa a esquema MVP con Supabase...\n"
    );
    ...
    const usuarios = await supabaseClient
      .from("usuarios")
      .select("count", { count: "exact" });
    const barberos = await supabaseClient
      .from("barberos")
      .select("count", { count: "exact" })
      .eq("activo", true);
    const servicios = await supabaseClient
      .from("servicios")
      .select("count", { count: "exact" })
      .eq("activo", true);
    const reservas = await supabaseClient
      .from("reservas")
      .select("count", { count: "exact" });
    ...
    if (barberosInfo && barberosInfo.length > 0) {
      ...
      barberosInfo.forEach((barbero) => {
        ...
        console.log(
          `  • ${barbero.nombre} (${info?.horario_inicio || "N/A"} - ${
            info?.horario_fin || "N/A"
          })`
        );
        console.log(
          `    Especialidades: ${info?.especialidades?.join(", ") || "N/A"}`
        );
      });
    }
    ...
    if (serviciosInfo && serviciosInfo.length > 0) {
      ...
      serviciosInfo.forEach((servicio) => {
        ...
        console.log(
          `  • ${servicio.nombre} - $${precioPesos} (${servicio.duracion} min)`
        );
      });
    }
...
async function runMigrationByParts() {
  try {
    ...
    console.log(
      "⏰ Por favor, ejecuta el siguiente script SQL manualmente en el editor SQL de Supabase:"
    );
    console.log("📁 Archivo: database/migration-mvp-complete.sql");
    console.log(
      "🌐 Dashboard: https://supabase.com/dashboard/project/qvxwfkbcrunaebahpmft/sql"
    );
