...
async function runMigration() {
  ...
  try {
    ...
    const barberos = await client.query('SELECT COUNT(*) FROM barberos WHERE activo = true');
    const servicios = await client.query('SELECT COUNT(*) FROM servicios WHERE activo = true');
    ...
    barberosInfo.rows.forEach(barbero => {
      console.log(`  • ${barbero.nombre} (${barbero.horario_inicio} - ${barbero.horario_fin})`);
    ...
    serviciosInfo.rows.forEach(servicio => {
      console.log(`  • ${servicio.nombre} - $${servicio.precio_pesos.toLocaleString()} (${servicio.duracion} min)`);
    });
